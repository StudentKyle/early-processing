...
class RobotLoader {
    ...
    try {
      ...
      while(line != null) {
        ...
        Robot robot = new Robot(
          ...
          int(p[2]), 
          */
          int(randomRobotXPos()), // x pos
          int(randomRobotYPos()), // y pos
          int(randomDirection()), // direction faced
          int(p[4]) // starting beeper amount
        );
  ...
  int randomRobotXPos() {
    float rand = random(11) + 1;
    int x = (int) rand;
    return x;
  }
  int randomRobotYPos() {
    float rand = random(10) +1;
    int y = (int) rand;
    return y;
  }
